...
progres.connect(connectionString, function (client) {
	...
	return [
		...
		// Inserting nothing should not give syntax error.
		function () {

			return client.insert(usersTable, []).then(function (insertedRows) {

				if (insertedRows.length !== 0) {

					throw new Error('Inserting an empty array should resolve with an empty array.');
				}
			});
		},
		...
		// Delete should resolve with the updated rows.
		function () {

			return client.delete(usersTable, {id: 2}).then(function (deletedRows) {

				if (!(
					deletedRows.length == 1 &&
					deletedRows[0].id == 2 &&
					deletedRows[0].name == 'neon'
				)) {

					throw new Error('Delete should resolve with the deleted rows.');
				}
			});
		}
	]
